...
Let's run it one more time to ensure nothing changed (*whole point of idempotency*).
...
> [!TIP]
> If you manage hosts with our Ansible collection, you can leverage the `crowdstrike.falcon.falcon_uninstall` role to automatically hide hosts after uninstalling the sensor by setting the `falcon_remove_host` to `true`.
